...
/*    
  ...
  def downloadNode(project) {
    ...
    final extractPath = nodeEnv.extractPath + '/' + nodeEnv.ver
    ant.mkdir(dir: extractPath)
    if(isWindows()) {
      // 32位
    ...
    ant.get(dest: extractPath, verbose: true) {
      // url(url: nodeUrl)
      url(url: npmUrl)
    }  
  }
...
/*  
  ...
  */
  ...
  static isOsX() {
    Os.isFamily(Os.FAMILY_WINDOWSx)
  }
